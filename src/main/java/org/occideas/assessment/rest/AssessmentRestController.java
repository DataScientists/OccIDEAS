...
public class AssessmentRestController {
    ...
    public Response exportAssessmentsNoiseCSV(FilterModuleVO filterModuleVO) {
        ...
        String fullPath = reportConfig.getExportDir() + reportHistoryVO.getId() + "_" + exportFileCSV;
    ...
    private ReportHistoryVO inProgress(ReportHistoryVO reportHistoryVO,
                                       Date date) {
        reportHistoryVO.setStatus(ReportsStatusEnum.IN_PROGRESS.getValue());
        reportHistoryVO.setProgress("0%");
        reportHistoryVO.setDuration(INITIAL_DURATION_MIN);
        reportHistoryVO.setStartDt(date);
        return reportHistoryService.saveNewTransaction(reportHistoryVO);
    }

    private ReportHistoryVO completeProgress(ReportHistoryVO reportHistoryVO,
                                             Date date,
                                             float duration,
                                             int count) {
    ...
    public Response exportInterviewRulesCSV(FilterAgentVO filterAgentVO) {
        ...
        try {
            inProgress(reportHistoryVO, Date.from(LocalDateTime.now().toInstant(ZoneOffset.UTC)));
            Integer count = reportHistoryService.generateInterviewRuleFilterReport(fullPath, filterAgentVO.getFilterAgent());
            long elapsedTime = System.currentTimeMillis() - startTime;
            completeProgress(reportHistoryVO, Date.from(LocalDateTime.now().toInstant(ZoneOffset.UTC)), elapsedTime, count);
        } catch (Exception e) {
