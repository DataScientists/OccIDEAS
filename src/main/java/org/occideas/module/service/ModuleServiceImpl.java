...
public class ModuleServiceImpl implements ModuleService {
    ...
    public List<ModuleVO> listAll() {
        return mapper.convertToModuleVOList(dao.findByDeleted(0), false);
    }

    @Override
    public List<ModuleVO> findById(Long id) {
        JobModule module = dao.getById(id);
    ...
    public long update(ModuleVO module) {
        ...
        dao.save(mapper.convertToModule(module, true));
    ...
    public long save(ModuleVO module) {
        JobModule jobModule = dao.save(mapper.convertToModule(module, true));
        return jobModule.getIdNode();
    }
    ...
    public NodeRuleHolder copyModule(ModuleCopyVO vo) {
        // todo refactor should not get max id
		...
		dao.save(mapper.convertToModule(copyVO, true));
    ...
    public NodeRuleHolder copyModuleAutoGenerateModule(ModuleCopyVO copyVo, ModuleReportVO report) {
		...
		dao.save(mapper.convertToModule(vo, true));
		return idNodeRuleHolder;
    }
    ...
    public NodeRuleHolder copyModuleAutoGenerateFragments(ModuleCopyVO vo, ModuleReportVO report,
        ...
        dao.save(mapper.convertToModule(vo.getVo(), true));
        return idNodeRuleHolder;
    }
    ...
    public ModuleVO getModuleByNameLength(String name, int length) {
        ...
        if (moduleName.isPresent()) {
            ...
            List<JobModule> modules = dao.findByNameStartsWith(name);
    ...
    public void manualBuildQSF(Long id, boolean filter) {
        List<ModuleVO> modules = this.findById(id);
        if (!modules.isEmpty()) {
            studyAgentUtil.buildQSF(modules.get(0), filter, false);
        }
    }
    ...
    @Override
    public Integer getModuleTranslationTotalCount(Long idNode) {
        final List<String> nodeList = dao.findDistinctNameByIdNode(idNode);
        if (nodeList.size() <= 1) {
    ...
    @Override
    public Integer getModuleTranslationCurrentCount(Long idNode, Long languageId) {
        List<String> nodeList = dao.findDistinctNameByIdNode(idNode);
    ...
    private Integer countUniqueNodeNamesInModule(Long idNode) {
        List<String> nodeList = dao.findDistinctNameByIdNode(idNode);
        CommonUtil.removeNonUniqueString(nodeList);
        return nodeList.size();
    }
    ...
    public List<LanguageModBreakdownVO> getModuleLanguageBreakdown(Long languageId) {
        ...
        for (JobModule module : listOfModuleIdNodes) {
            ...
            Integer currentCount = getModuleTranslationCurrentCount(idNode, languageId);
            Integer totalCount = getModuleTranslationTotalCount(idNode);
    ...
    public Integer getTotalUntranslatedModule(Long languageId) {
        ...
        int count = 0;
        for (JobModule module : listOfModuleIdNodes) {
            count = count + countUniqueNodeNamesInModule(module.getIdNode());
        }
        return count;
    }
    ...
    public Integer getModulesWithTranslationCount(long languageId) {
        ...
        CommonUtil.replaceListWithLowerCaseAndTrim(nodeLanguageList);
        for (JobModule module : listOfModuleIdNodes) {
            if (hasAnyNodeTranslated(module.getIdNode(), languageId, nodeLanguageList)) {
                count++;
            }
        }
        return count;
    ...
    public Integer getTotalTranslatedNodeByLanguage(long languageId) {
        ...
        CommonUtil.replaceListWithLowerCaseAndTrim(nodeLanguageList);
        for (JobModule module : listOfModuleIdNodes) {
            List<String> nodeList = dao.findDistinctNameByIdNode(module.getIdNode());
            CommonUtil.removeNonUniqueString(nodeList);
            CommonUtil.replaceListWithLowerCaseAndTrim(nodeList);
            for (int i = 0; i < nodeList.size(); i++) {
                if (nodeLanguageList.contains(nodeList.get(i).toLowerCase().trim())) {
