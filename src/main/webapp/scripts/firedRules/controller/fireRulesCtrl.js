                ...
                for(var i = 0; i < interviewFiredRules.length; i++) {
                  ...
                  for(var j = 0; j < rules.length; j++) {
                    ...
                    var agentCount = _.find($scope.data.agentCount,
                      function(o) {
                        return o.idAgent == rules[j].agentId;
                      });
                    if(agentCount) {
          ...
          for(var i = 0; i < ruleArray.length; i++) {
            ...
            for(var x = 0; x < scope.rule.conditions.length; x++) {
              ...
              var topMod = _.find($scope.data.topModuleNameList,
                function(o) {
                  return o.idnode == condition.topNodeId;
                });
              if(topMod) {
                  ...
                  if(bFoundNoiseRules) {
                    ...
                    for(var k = 0; k < noiseRules.length; k++) {
                      ...
                      if(noiseRule.type == 'BACKGROUND') {
                        ...
                        var noiseRow = {
                          nodeNumber: noiseRule.conditions[0].number,
                ...
                if(response.status == '200') {
                  ...
                  if(bFoundVibrationRules) {
                    ...
                    for(var k = 0; k < vibrationRules.length; k++) {
                      ...
                      var vibrationRow = {
                        nodeNumber: vibrationRule.conditions[0].number,
                  ...
                  for(var j = 0; j < $scope.data.firedRules.length; j++) {
                    ...
                    if(agentAssessing.idAgent == firedRule.agent.idAgent) {
                      if(firedRule.levelValue < rule.levelValue) {
                        //rule = firedRule;
                        rule = {
                          agentId: agentAssessing.idAgent,
                          level: firedRule.level,
                          levelValue: firedRule.levelValue
                        }
                      }
                    }
                  }
        ...
        ['Use Auto', function($itemScope, $event, model) {
          ...
          for(var i = 0; i < assessments.length; i++) {
            ...
            var manualAssessment = {
              ...
              levelValue: assessment.levelValue
            };
