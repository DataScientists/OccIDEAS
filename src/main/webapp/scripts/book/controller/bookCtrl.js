(function () {
    ...
    BookCtrl.$inject = ['BookService', 'DifferenceUtil', 'NgTableParams', '$state', '$scope', '$filter', '$mdDialog', 'Upload', '$timeout'];


    function BookCtrl(BookService, DifferenceUtil, NgTableParams, $state, $scope, $filter, $mdDialog, Upload, $timeout) {
        const self = this;
        ...
        self.uploadJson = (group) => {
            console.log('uploadJson', group);
            self.uploadJsonActions = {
                newModule: {},
                uploadJsonSave() {
                    const file = $scope.f;
                    if (file) {
                        file.upload = Upload.upload({
                            url: 'web/rest/book/uploadJsonToBook',
                            file,
                            data: {
                                bookId: group.data[0].id,
            ...
            $mdDialog.show({
                scope: $scope,
                preserveScope: true,
                templateUrl: 'scripts/book/partials/uploadJsonDialog.html',
                clickOutsideToClose: false
            });
        }
